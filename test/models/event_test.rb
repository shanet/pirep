...
class EventTest < ActiveSupport::TestCase
  ...
  test 'next start date for DST forward boundary' do
    travel_to(Time.zone.local(2023, 3, 1)) do
      event = create(:event, :recurring, start_date: Time.zone.local(2023, 3, 1), end_date: Time.zone.local(2023, 3, 2), recurring_cadence: :monthly)
      ...
      assert_not start_date.dst?, 'Event starting during DST'
      assert next_start_date.dst?, 'Event\'s next start date not during DST'
  ...
  test 'next start date for DST backward boundary' do
    travel_to(Time.zone.local(2023, 11, 1)) do
